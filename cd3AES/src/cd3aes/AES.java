package cd3aes;

//Copyright Popa Tiberiu 2011
//free to use this in any way

//package tsd.crypto.algoritm;

import cd3aes.*;
import javax.crypto.KeyGenerator;
import javax.crypto.SecretKey;
import java.io.*;

public class AES {
    //Nb so cot trang thai(state) 
    //Nk so luong tu 32bit trong khoa
        //Nếu Nk = 4 thì số lượng vòng lặp sẽ là 10 vòng (rounds), 
            //Nk = 6 thì số lượng vòng lặp sẽ là 12 vòng, và Nk = 8 thì số lượng vòng lặp sẽ là 14 vòng.
    //Nr so luong vong lap trong AES cipher
    private static int Nb, Nk, Nr; 
    private static byte[][] w; //luu tru khóa con dc generate ra
    static double t1, t2;
    
    public AES(){
        
    }
    
    private static int[] sbox = {};

    private static int[] inv_sbox = {};
    //Rcon (Round Constant) để tạo ra các giá trị khóa con trong quá trình mở rộng khóa (key expansion)
    private static int Rcon[] = {
        };

    public static byte[] keygeneration(int num) throws Exception {

        KeyGenerator generator = KeyGenerator.getInstance("AES");
        generator.init(num);
        SecretKey key = generator.generateKey();
        return key.getEncoded();

    }

    private static byte[] xor_func(byte[] a, byte[] b) { //1 (hoặc true) nếu hai bit khác nhau, ngược lại trả về 0 (hoặc false) nếu hai bit giống nhau
        byte[] out = new byte[a.length];
        for (int i = 0; i < a.length; i++) {
            out[i] = (byte) (a[i] ^ b[i]);
        }
        return out;

    }

    private static byte[][] generateSubkeys(byte[] key) { //Keyexpansion
        byte[][] tmp = new byte[Nb * (Nr + 1)][4];

        int i = 0;
        while (i < Nk) { 

            tmp[i][0] = key[i * 4];
            tmp[i][1] = key[i * 4 + 1];
            tmp[i][2] = key[i * 4 + 2];
            tmp[i][3] = key[i * 4 + 3];
            i++;
        }
        i = Nk;
        while (i < Nb * (Nr + 1)) {
            byte[] temp = new byte[4];
            for (int k = 0; k < 4; k++) {
                temp[k] = tmp[i - 1][k];
            }
            if (i % Nk == 0) {
                temp = SubWord(rotateWord(temp));
                temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
            } else if (Nk > 6 && i % Nk == 4) {
                temp = SubWord(temp);
            }
            tmp[i] = xor_func(tmp[i - Nk], temp);
            i++;
        }

        return tmp;
    }

    private static byte[] SubWord(byte[] in) {//từ (word) 4-byte thành các byte riêng lẻ,áp dung sbox len moi byte
        byte[] tmp = new byte[in.length];
        for (int i = 0; i < tmp.length; i++) {
            tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);
        }

        return tmp;
    }

    private static byte[] rotateWord(byte[] input) { //Rotword
        byte[] tmp = new byte[input.length];
        tmp[0] = input[1];
        tmp[1] = input[2];
        tmp[2] = input[3];
        tmp[3] = input[0];

        return tmp;
    }

    private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) { //thêm khóa con vào ma trận trạng thái trong quá trình mã hóa hoặc giải mã

        byte[][] tmp = new byte[state.length][state[0].length];

        for (int c = 0; c < Nb; c++) {
            for (int l = 0; l < 4; l++) {
                tmp[l][c] = (byte) (state[l][c] ^ w[round * Nb + c][l]);
            }
        }

        return tmp;
    }

    private static byte[][] SubBytes(byte[][] state) { //thay the gia tri tai vi tri [][] bang gtri tai vi tri tuong ung trong sbox

        byte[][] tmp = new byte[state.length][state[0].length];
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < Nb; col++) {
                tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);
            }
        }

        return tmp;
    }

    private static byte[][] InvSubBytes(byte[][] state) {
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < Nb; col++) {
                state[row][col] = (byte) (inv_sbox[(state[row][col] & 0x000000ff)] & 0xff);
            }
        }

        return state;
    }

    private static byte[][] ShiftRows(byte[][] state) {

        byte[] t = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                t[c] = state[r][(c + r) % Nb];
            }
            for (int c = 0; c < Nb; c++) {
                state[r][c] = t[c];
            }
        }

        return state;
    }

    private static byte[][] InvShiftRows(byte[][] state) {
        byte[] t = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                t[(c + r) % Nb] = state[r][c];
            }
            for (int c = 0; c < Nb; c++) {
                state[r][c] = t[c];
            }
        }
        return state;
    }

    private static byte[][] InvMixColumns(byte[][] s) {
        int[] sp = new int[4];
        byte b02 = (byte) 0x0e, b03 = (byte) 0x0b, b04 = (byte) 0x0d, b05 = (byte) 0x09;
        for (int c = 0; c < 4; c++) {
            sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04, s[2][c]) ^ FFMul(b05, s[3][c]);
            sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ FFMul(b04, s[3][c]);
            sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
            sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05, s[2][c]) ^ FFMul(b02, s[3][c]);
            for (int i = 0; i < 4; i++) {
                s[i][c] = (byte) (sp[i]);
            }
        }

        return s;
    }

    private static byte[][] MixColumns(byte[][] s) {
        int[] sp = new int[4];
        byte b02 = (byte) 0x02, b03 = (byte) 0x03;
        for (int c = 0; c < 4; c++) {
            sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c]; //XOR
            sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
            sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
            sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
            for (int i = 0; i < 4; i++) {
                s[i][c] = (byte) (sp[i]);
            }
        }

        return s;
    }

    public static byte FFMul(byte a, byte b) {
        byte aa = a, bb = b, r = 0, t;
        while (aa != 0) {
            if ((aa & 1) != 0) {
                r = (byte) (r ^ bb);
            }
            t = (byte) (bb & 0x80);
            bb = (byte) (bb << 1);
            if (t != 0) {
                bb = (byte) (bb ^ 0x1b);
            }
            aa = (byte) ((aa & 0xff) >> 1);
        }
        return r;
    }

    public static byte[] encryptBloc(byte[] in) {
        byte[] tmp = new byte[in.length];

        byte[][] state = new byte[4][Nb];

        for (int i = 0; i < in.length; i++) { //thành ma trận state có kích thước 4x4 (4 hàng, 4 cột) 
            state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
        }

        state = AddRoundKey(state, w, 0);
        for (int round = 1; round < Nr; round++) {
            state = SubBytes(state);
            state = ShiftRows(state);
            state = MixColumns(state);
            state = AddRoundKey(state, w, round);
        }
        state = SubBytes(state);
        state = ShiftRows(state);
        state = AddRoundKey(state, w, Nr);

        for (int i = 0; i < tmp.length; i++) {
            tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
        }

        return tmp;
    }
    
    //Trong hàm này, khối dữ liệu được chuyển đổi thành một ma trận state có kích thước 4x4. 
    //Sau đó, hàm thực hiện các bước giải mã tương ứng với các bước mã hóa, nhưng trong thứ tự ngược lại. 
      //Cụ thể, các bước được thực hiện như sau:
//AddRoundKey: Sử dụng phần tử khóa w tương ứng với vòng lặp thứ Nr để thêm (XOR) với ma trận state.
//Với mỗi round từ Nr - 1 đến 1:
//InvSubBytes: Thay thế mỗi phần tử trong ma trận state bằng phần tử tương ứng trong bảng thay thế ngược invSbox.
//InvShiftRows: Dịch chuyển các hàng của ma trận state sang phải (với hàng thứ i được dịch chuyển i vị trí).
//AddRoundKey: Sử dụng phần tử khóa w tương ứng với vòng lặp thứ round để thêm (XOR) với ma trận state.
//InvMixColumns: Thực hiện phép nhân ma trận state với ma trận InvMixColumnMatrix.
//Cuối cùng, sau khi thực hiện round cuối cùng, thực hiện các bước như sau:
//InvSubBytes: Thay thế mỗi phần tử trong ma trận state bằng phần tử tương ứng trong bảng thay thế ngược invSbox.
//InvShiftRows: Dịch chuyển các hàng của ma trận state sang phải (với hàng thứ i được dịch chuyển i vị trí).
//AddRoundKey: Sử dụng phần tử khóa w tương ứng với vòng lặp đầu tiên để thêm (XOR) với ma trận state.
//Sau đó, ma trận state được chuyển đổi trở lại thành một mảng byte, và trả về khối dữ liệu đã được giải mã.
    public static byte[] decryptBloc(byte[] in) {
        byte[] tmp = new byte[in.length];

        byte[][] state = new byte[4][Nb];

        for (int i = 0; i < in.length; i++) { //chuyển đổi khối dữ liệu input thành ma trận state có kích thước 4x4 (4 hàng, 4 cột)
            state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
        }

        state = AddRoundKey(state, w, Nr);
        for (int round = Nr - 1; round >= 1; round--) {
            state = InvSubBytes(state);
            state = InvShiftRows(state);
            state = AddRoundKey(state, w, round);
            state = InvMixColumns(state);

        }
        state = InvSubBytes(state);
        state = InvShiftRows(state);
        state = AddRoundKey(state, w, 0);

        for (int i = 0; i < tmp.length; i++) {
            tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
        }

        return tmp;
    }

    public static byte[] encrypt(byte[] in, byte[] key) {

        Nb = 4;
        Nk = key.length / 4;
        Nr = Nk + 6;

        int lenght = 0;
        byte[] padding = new byte[1];
        int i;
        lenght = 16 - in.length % 16;
        padding = new byte[lenght];
        padding[0] = (byte) 0x80;

        for (i = 1; i < lenght; i++) {
            padding[i] = 0;
        }

        byte[] tmp = new byte[in.length + lenght];
        byte[] bloc = new byte[16];

        w = generateSubkeys(key);

        int count = 0;

        for (i = 0; i < in.length + lenght; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = encryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length) {
                bloc[i % 16] = in[i];
            } else {
                bloc[i % 16] = padding[count % 16];
                count++;
            }
        }
        if (bloc.length == 16) {
            bloc = encryptBloc(bloc);
            System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
        }

        return tmp;
    }

    public static byte[] decrypt(byte[] in, byte[] key) {
        int i;
        byte[] tmp = new byte[in.length];
        byte[] bloc = new byte[16];

        Nb = 4;
        Nk = key.length / 4;
        Nr = Nk + 6;
        w = generateSubkeys(key);

        for (i = 0; i < in.length; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = decryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length) {
                bloc[i % 16] = in[i];
            }
        }
        bloc = decryptBloc(bloc);
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);

        tmp = deletePadding(tmp);

        return tmp;
    }

    private static byte[] deletePadding(byte[] input) { //xóa byte giá tri 0 o cuoi chuoi
        int count = 0;

        int i = input.length - 1;
        while (input[i] == 0) {
            count++;
            i--;
        }

        byte[] tmp = new byte[input.length - count - 1];
        System.arraycopy(input, 0, tmp, 0, tmp.length);
        return tmp;
    }

    public static byte[] encrypt(byte[] in, byte[] key, int n) {

        t1 = System.nanoTime();
        Nb = 4; //socottrangthai
        Nk = key.length / 4; //moiwordgom4byte
        Nr = n;

        int lenght = 0; //thêm các byte padding cần thiết cho in để đảm bảo kích thước của nó là một bội số của 16 byte
        byte[] padding = new byte[1]; 
        int i;
        lenght = 16 - in.length % 16;
        padding = new byte[lenght];
        padding[0] = (byte) 0x80;

        for (i = 1; i < lenght; i++) {
            padding[i] = 0;
        }

        byte[] tmp = new byte[in.length + lenght];
        byte[] bloc = new byte[16];

        w = generateSubkeys(key);

        int count = 0;

        for (i = 0; i < in.length + lenght; i++) {  //chia thành các kh?i 16byte de ma hoa
            if (i > 0 && i % 16 == 0) {
                bloc = encryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length) {
                bloc[i % 16] = in[i];
            } else {
                bloc[i % 16] = padding[count % 16];
                count++;
            }
        }

        if (bloc.length == 16) {
            bloc = encryptBloc(bloc);
            System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
        }
        t2 = System.nanoTime();
        return tmp;
    }

    public static byte[] decrypt(byte[] in, byte[] key, int n) {

        t1 = System.nanoTime();
        int i;
        byte[] tmp = new byte[in.length];
        byte[] bloc = new byte[16];

        Nb = 4;
        Nk = key.length / 4;
        Nr = n;
        w = generateSubkeys(key);

        for (i = 0; i < in.length; i++) { //chiathanhcackhoi16bytedegiaima
            if (i > 0 && i % 16 == 0) {
                bloc = decryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length) {
                bloc[i % 16] = in[i];
            }
        }
        bloc = decryptBloc(bloc);
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);

        tmp = deletePadding(tmp); //bocacbytepadding
        t2 = System.nanoTime();
        return tmp;
    }

}
